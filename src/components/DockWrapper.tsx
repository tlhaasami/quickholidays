...
export function DockWrapper() {
  ...
  return (
    <>
      ...
      {/* Mobile Dock — appears from behind hamburger, desktop hidden */}
      <AnimatePresence>
        {showMobileDock && (
          <motion.div
            key="mobile-dock"
            ...
            className="fixed top-16 right-4 z-[999] flex flex-col items-end md:hidden"
          >
            <div className="bg-white/95 dark:bg-zinc-950/95 border border-zinc-200 dark:border-white/10 shadow-2xl p-2 rounded-3xl flex flex-col items-center gap-1.5">
              {items
                .filter((i) => i.id !== "whatsapp" && i.id !== "ai-assistant")
                .map((item) => (
                  <div
                    key={item.id}
                    ...
                    className={`flex flex-col items-center justify-center gap-0.5 px-3 py-2 rounded-2xl transition-all cursor-pointer select-none outline-none ${
                      item.isActive
                        ? "bg-[#C99537]/15 text-[#C99537] dark:text-[#E2B755]"
                        : "text-zinc-500 dark:text-zinc-400 hover:text-[#C99537] dark:hover:text-[#E2B755]"
                    }`}
                  >
                    <div className={`${item.id === "theme-toggle" ? "w-10 h-10" : "w-5 h-5"} flex items-center justify-center shrink-0`}>
                      {item.icon}
                    </div>
