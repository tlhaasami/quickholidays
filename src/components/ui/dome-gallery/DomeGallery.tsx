...
const getFlagSrc = (src: string): string => {
  const parts = src.split('/');
  if (parts.length >= 3) {
    const slug = parts[2];
    return `/flags/${slug}/flag-${slug}.webp`;
  }
  return '';
};
interface GridCoord {
...
function buildItems(pool: (string | ImageItem)[], seg: number) {
  ...
  const normalizedImages = pool.map(image => {
    if (typeof image === 'string') {
      return { src: image, alt: '', country: '', info: '' };
    }
    return { 
      src: image.src || '', 
      alt: image.alt || '',
      country: (image as any).country || '',
      info: (image as any).info || ''
    };
  });
...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    let rafId: number;
    const spinSpeed = 0.05; // 0.05 degrees per frame
    
    const animate = () => {
      const isPointerDown = pointerDownRef.current;
      const isDragging = draggingRef.current;
      const isInertia = inertiaRAF.current !== null;
      const isEnlarged = rootRef.current?.getAttribute('data-enlarging') === 'true';

      if (!isPointerDown && !isDragging && !isInertia && !isEnlarged) {
        rotationRef.current.y = wrapAngleSigned(rotationRef.current.y + spinSpeed);
        applyTransform(rotationRef.current.x, rotationRef.current.y);
      }
  ...
  const openItemFromElement = useCallback(
    (el: HTMLElement) => {
      ...
      const matched = items.find(it => it.src === rawSrc);
      if (matched) {
        setActivePlace(matched);
      }

      // 50ms delay lets React commit the details panel state to the DOM and reflow the flex container.
      // This ensures frameRef.current is measured at its correct shifted position!
      setTimeout(() => {
    ...
    [enlargeTransitionMs, lockScroll, openedImageHeight, openedImageWidth, segments, unlockScroll, items]
  );
