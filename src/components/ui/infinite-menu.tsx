...
class InfiniteGridMenu {
  ...
  public flagSizeFactor = 0.28;
  public borderColor = '#CCA352';
  public borderWidth = 12;
  ...
  constructor(
    ...
    this.flagSizeFactor = flagSizeFactor;
  ...
  public setBorder(color: string, width: number): void {
    if (this.borderColor !== color || this.borderWidth !== width) {
  ...
  private initTexture(): void {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        // Draw the ring border using stroke
        ctx.save();
        ctx.strokeStyle = this.borderColor;
        ctx.lineWidth = this.borderWidth;
        const radius = cellSize / 2 - this.borderWidth / 2 - 8;
        ctx.beginPath();
        ctx.arc(x + cellSize / 2, y + cellSize / 2, radius, 0, Math.PI * 2);
        ctx.stroke();

        // Clip and draw the circular flag image inside the border
        const clipRadius = radius - this.borderWidth / 2 - 2;
        ctx.beginPath();
        ctx.arc(x + cellSize / 2, y + cellSize / 2, clipRadius, 0, Math.PI * 2);
        ctx.clip();
        const flagSize = clipRadius * 2;
        const flagX = x + cellSize / 2 - clipRadius;
        const flagY = y + cellSize / 2 - clipRadius;
  ...
  private updateProjectionMatrix(): void {
    ...
    const height = this.SPHERE_RADIUS * this.flagSizeFactor;
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({
  ...
  useEffect(() => {
    if (sketchRef.current) {
      sketchRef.current.flagSizeFactor = flagSizeFactor;
      sketchRef.current.setBorder(borderColor, borderWidth);
      sketchRef.current.setScale(scale);
      sketchRef.current.resize();
    }
  }, [flagSizeFactor, borderColor, borderWidth, scale]);
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      ...
      if (lastIndexRef.current !== itemIndex) {
  ...
  }, [items, scale, flagSizeFactor, borderColor, borderWidth]);
        ...
        <>
          ...
          <p
            className={`
              ...
              text-sm sm:text-lg
              top-1/2
              right-6 sm:right-12 md:right-[10%] lg:right-[14%]
              transition-all
              ease-[cubic-bezier(0.25,0.1,0.25,1.0)]
              text-zinc-550 dark:text-zinc-400
              ${isMoving
                ? 'opacity-0 pointer-events-none duration-[100ms] -translate-x-4 -translate-y-1/2'
