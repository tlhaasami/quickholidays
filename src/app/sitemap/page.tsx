  ...
  return (
    <div className="bg-white dark:bg-black min-h-screen text-zinc-950 dark:text-white pt-32 pb-24 px-8 sm:px-16 md:px-24 transition-colors duration-300">
      <div className="max-w-4xl mx-auto text-left">
        ...
        <div className="border-b border-zinc-200 dark:border-white/10 pb-6 mb-12">
          <motion.h1
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 font-sans">
          ...
          <div className="space-y-4 text-left">
            <h3 className="font-serif text-xl sm:text-2xl text-zinc-900 dark:text-white font-semibold border-b border-zinc-200 dark:border-white/5 pb-2">Main Pages</h3>
            <ul className="space-y-2 text-sm sm:text-base font-light text-zinc-650 dark:text-zinc-400 font-sans">
              {pages.map((p) => (
                <li key={p.href}>
                  <Link href={p.href} className="hover:text-primary transition-colors flex items-center gap-2">
                    <span className="text-primary text-[10px]">■</span>
                    <span>{p.name}</span>
                    <span className="text-zinc-500 text-xs font-light">({p.href})</span>
                  </Link>
                </li>
              ))}
            </ul>
          ...
          <div className="space-y-4 text-left">
            <h3 className="font-serif text-xl sm:text-2xl text-zinc-900 dark:text-white font-semibold border-b border-zinc-200 dark:border-white/5 pb-2">Country Visa Guides</h3>
            <ul className="space-y-2 text-sm sm:text-base font-light text-zinc-650 dark:text-zinc-400 max-h-96 overflow-y-auto pr-2 no-scrollbar font-sans">
              {COUNTRIES.map((c) => (
                <li key={c.slug}>
                  <Link href={`/schengen-visa/${c.slug}`} className="hover:text-primary transition-colors flex items-center gap-2">
                    ...
                    <span className="text-zinc-500 text-xs font-light">(/schengen-visa/{c.slug})</span>
                  </Link>
                </li>
              ))}
            </ul>
